 ...
 *
 */
public class FillNeo4jDb_3A_All {
...
//		File neo4jDir = new File("C:/Users/denise.slenter/AppData/Roaming/Neo4j Desktop/Application/neo4jDatabases/database-d4848fea-f97b-4eed-9ea1-e5ff83eca022/installation-3.3.1");
		...
		// file is created in step 2 (FilterInteractionsAll_2.java)
		File network = new File("/home/deniseslenter/git/DirectedSmallMoleculesNetwork/Code/DSMNProject/network-wdAll-20201116-wpallDev.txt");
		
		FillNeo4jDb_3A_All neo4jdb = new FillNeo4jDb_3A_All(uri, network, neo4jDir);
		neo4jdb.loadNetwork();
	}
	...
	public FillNeo4jDb_3A_All(String uri, File network, File neo4jDir) throws URISyntaxException {
...
//		driver = GraphDatabase.driver(uri); //use this line when no authentication is needed for Neo4j
	...
	public void loadNetwork() throws Exception {
		System.out.println("[INFO]\tClear database - make sure no old data is in the database.");
		...
		try(Session session = driver.session()) {
			...
			try (Transaction tx = session.beginTransaction()) {
				System.out.println("Testing directory for source->reaction->target: " + neo4jDir.getAbsolutePath());
				...
				tx.run("LOAD CSV WITH HEADERS FROM 'file:///edges.txt' AS line FIELDTERMINATOR '\t' " +
						...
						"CREATE (source)-[:AllInteractions {intType: line.IntTypes, intRef: line.interactionRef, occurences: toInteger(line.Occurences), count: toInteger(line.CountPathways), pathways: line.Pathways, pathwayName: line.PathwayName, proteinName: line.proteinName, pwont: line.pathwayOntology, disont: line.diseaseOntology, pwRef: line.pathwayRef, proteinName: line.proteinName, rheaID: line.rheaIDs, sourceRef: line.sourceRef, targetRef: line.targetRef}]->(reaction) " +
						"CREATE (reaction)-[:AllInteractions {intType: line.IntTypes, intRef: line.interactionRef, occurences: toInteger(line.Occurences), count: toInteger(line.CountPathways), pathways: line.Pathways, pathwayName: line.PathwayName, proteinName: line.proteinName, pwont: line.pathwayOntology, disont: line.diseaseOntology, pwRef: line.pathwayRef, proteinName: line.proteinName, rheaID: line.rheaIDs, sourceRef: line.sourceRef, targetRef: line.targetRef}]->(target) " );
		...
		try(Session session = driver.session()) {
            ...
            while (result2.hasNext())
            {
             ...
             // count AllInteraction edges
             StatementResult result5 = session.run("MATCH ()-[r:AllInteractions]->() RETURN count(r) AS count");
             int countAll = 0;
             while (result5.hasNext())
             {
                 Record record = result5.next();
                 countAll = record.get("count").asInt();
             }
            
            
            // count Unconnected Nodes
            StatementResult result4 = session.run("MATCH (n:Metabolite) WHERE NOT (n)-[:AllInteractions|:AllCatalysis]-() RETURN count(n) AS count");
 			int countU = 0;
            ...
            System.out.println("[INFO]\tDatabase contains " + countAll + " AllInteraction edges.");
            System.out.println("[INFO]\tDatabase contains " + countU + " Unconnected nodes (with relationship label :AllInteractions or :AllCatalysis.");
            System.out.println("[INFO]\tDSMN Step 3 All reactions done.");
		}
	}
}
