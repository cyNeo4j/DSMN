 ...
 *
 */
public class FillNeo4jDb_3C_Reactome {
...
//		File neo4jDir = new File("C:/Users/denise.slenter/AppData/Roaming/Neo4j Desktop/Application/neo4jDatabases/database-d4848fea-f97b-4eed-9ea1-e5ff83eca022/installation-3.3.1");
		...
		File network = new File("/home/deniseslenter/git/DirectedSmallMoleculesNetwork/Code/DSMNProject/network-wdReactome-20201116-wpallDev.txt");
		
		FillNeo4jDb_3C_Reactome neo4jdb = new FillNeo4jDb_3C_Reactome(uri, network, neo4jDir);
		neo4jdb.loadNetwork();
	}
	...
	public FillNeo4jDb_3C_Reactome(String uri, File network, File neo4jDir) throws URISyntaxException {
		this.network = network;
		driver = GraphDatabase.driver(uri, AuthTokens.basic("neo4j", "dsmn") );
//		driver = GraphDatabase.driver(uri);
		this.neo4jDir = neo4jDir;
		System.out.println("Directory of Neo4j for step 3D: " + this.neo4jDir.getAbsolutePath());
	}
	...
	public void loadNetwork() throws Exception {
		...
		try(Session session = driver.session()) {
			try (Transaction tx = session.beginTransaction()) {
				System.out.println("Testing directory for source->Reactomereaction->target: " + neo4jDir.getAbsolutePath());
		...
		try(Session session = driver.session()) {
         ...
         // count Reactome edges
            StatementResult result5 = session.run("MATCH ()-[r:ReactomeInteractions]->() RETURN count(r) AS count");
			int countR = 0;
            ...
            System.out.println("[INFO]\tDSMN Step 3 Reactome interactions done.");
		}
	}
}
