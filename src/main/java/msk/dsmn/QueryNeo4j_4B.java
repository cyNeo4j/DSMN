...
public class QueryNeo4j_4B {

	public static void main(String[] args) throws Exception {
		// make sure Neo4j is started and located at this URI
		String uri = "bolt://localhost:7687";
			
		File queryList = new File("query404-metabolites-wd.txt"); //change "wd" with "chebi" to test the mapping approach.
		
		QueryNeo4j_4B qn = new QueryNeo4j_4B(uri);
		qn.generateSubNetwork(queryList);
	}
	
	private Driver driver;
	
	public QueryNeo4j_4B(String uri) {
	...
	public void generateSubNetwork(File queryList) throws Exception {
		...
		File output = new File("subnetwork-wd-404-MAR2021.txt"); //("subnetwork-wd-Rist-20201116-wpallDEV.txt");
		...
		if(new String(queryArray).contains("Q")) { //Execute 1st query (directed) to obtain metabolite1->reactionID->metabolite2 info
...
//			System.out.println(path.start().get("id").asString().replace("\"", "") + "\t" + path.end().get("id").asString().replace("\"", "") + "\t" + path.length());
		...
		//Execute 2nd query (undirected) to obtain enzymeID->reactionID->metabolite2 info

		String neo4jQueryEnz = "MATCH (n:Metabolite) where n.wdID IN " + queryArray + " WITH collect(n) as nodes " +      //[*] for all interactions
				" UNWIND nodes as n " +
				" UNWIND nodes as m " +
				" WITH * WHERE n <> m " +
				...
				" RETURN p "; 
		System.out.println("[INFO]\tRunning Neo4j query:\n" + neo4jQueryEnz);
		StatementResult result3 = session.run(neo4jQueryEnz);
		List<Record> resultList3 = result3.list();
		Map<Node, Set<Node>> interactions3 = new HashMap<Node, Set<Node>>();
		System.out.println("[INFO]\tRetrieving Enzymes.");
		
		for(Record record : resultList3) {
			PathValue p = (PathValue)record.get("p");
			Path path = p.asPath();
			Map<Long, Node> nodes = new HashMap<Long, Node>();
			Map<Long, Relationship> relationships = new HashMap<Long, Relationship>();
...
//			System.out.println(path.start().get("id").asString().replace("\"", "") + "\t" + path.end().get("id").asString().replace("\"", "") + "\t" + path.length());
			for(Relationship r : path.relationships()) {
				...
				if(!interactions3.containsKey(start)) {
					interactions3.put(start, new HashSet<Node>());
					interactions3.get(start).add(end);
				//When a startnode is an enzyme
			
					if(start.get("wdID").isNull() && start.get("rwID").isNull()) { 
					writer.write(start.get("enID").asString().replace("\"", "") + "\t" + end.get("rwID").asString().replace("\"", "") + "\t" + "1" + "\t" + "1" + "\t"
								+ r.get("pathways").asString().replace("\"", "") + "\t" + "[Enzyme Conversion]" + "\t" + r.get("pathwayName").asString().replace("\"","") + "\t" + r.get("proteinName").asString().replace("\"", "")  
								+ r.get("proteinId").asString().replace("\"", "") + "\t" 
								+ "[NA]" + "\t" + "[NA]" + "\t" + "[NA]" + "\t" + "[NA]" + "\t" + "[NA]" +
							"\n" ); 
					}
				}	
					...
					if(!interactions3.get(start).contains(end)) {
						interactions3.get(start).add(end); 
		...
		//TO-DO add enzyme query + data workaround here.
			...
			if(!interactions.containsKey(start)) {
				...
				//When a startnode is a reaction node, not a metabolite or enzyme
					...
					writer.write(start.get("rwID").asString().replace("\"", "") + "\t" + end.get("wdID").asString().replace("\"", "") +
						...
						/*
						 * //When a startnode is an enzyme else {
						 * writer.write(start.get("enID").asString().replace("\"", "") + "\t" +
						 * end.get("rwID").asString().replace("\"", "") + "\t" + "1" + "\t" + "1" + "\t"
						 * + r.get("pathways").asString().replace("\"", "") + "\t" +
						 * "[Enzyme Conversion]" + "\t" + r.get("pathwayName").asString().replace("\"",
						 * "") + "\t" + r.get("proteinName").asString().replace("\"", "") + //"\t" +
						 * r.get("proteinId").asString().replace("\"", "") + "\t" +
						 * //r.get("interactionId").asString().replace("\"", "") + "\t" + "[NA]" + "\t"
						 * + "[NA]" + "\t" + "[NA]" + "\t" + "[NA]" + "\t" + "[NA]" + "\t" + "[NA]" +
						 * "\n" ); }
						 */
