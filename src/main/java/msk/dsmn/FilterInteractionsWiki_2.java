 ...
 * 
 */
public class FilterInteractionsWiki_2 {
	
	public static void main(String[] args) throws Exception {
		
		// run SparqlQuery_1a_Jenkins to create this file (change name accordingly)
		File sparqlResult = new File("sparql-resultA-wdWiki-20201116-wpallDev.txt"); 
	
		// run SparqlQuery_1b_Jenkins to create this file (change name accordingly)
		File sparqlResultB = new File("sparql-resultB-wdWiki-20201116-wpallDev.txt"); 
		
		// run SparqlQuery_1c_Jenkins to create this file (change name accordingly)
		File sparqlResultC = new File("sparql-resultC-wdWiki-20201116-wpallDev.txt"); 
		...
		FilterInteractionsWiki_2 fi = new FilterInteractionsWiki_2(sparqlResult, sparqlResultC, sparqlResultB, sideMetabolites);

		// output files (add date and change name accordingly)
		File network = new File("network-wdWiki-20201116-wpallDev.txt");
		File networkFiltered = new File("network-filtered-wdWiki-20201116-wpallDev.txt");
	...
	public FilterInteractionsWiki_2(File sparqlResult, File sparqlResultC, File sparqlResultB, File sideMetabolites) {
	...
	public void createNetworkFiles(File network, File networkFiltered) throws Exception {
		...
		while((line = reader.readLine()) != null) { //if a  line is not empty
			...
			if((!interactionID.isEmpty()) && (map.get(key).getInteractionID().isEmpty())){
				...
				map.get(key).getInteractionID().add("ReactionID_Wiki:" + count); //Only add a reaction ID number, if none is present.
