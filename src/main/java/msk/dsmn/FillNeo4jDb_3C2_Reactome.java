 ...
 *
 */
public class FillNeo4jDb_3C2_Reactome {
...
//		File neo4jDir = new File("C:/Users/denise.slenter/AppData/Roaming/Neo4j Desktop/Application/neo4jDatabases/database-d4848fea-f97b-4eed-9ea1-e5ff83eca022/installation-3.3.1");
		...
		FillNeo4jDb_3C2_Reactome neo4jdb = new FillNeo4jDb_3C2_Reactome(uri, network, neo4jDir);
		neo4jdb.loadNetwork();
	}
	...
	public FillNeo4jDb_3C2_Reactome(String uri, File network, File neo4jDir) throws URISyntaxException {
	...
	public void loadNetwork() throws Exception {
		...
		try(Session session = driver.session()) {
			try (Transaction tx = session.beginTransaction()) {
				System.out.println("Testing directory for source->ReactomeAnalysisReaction->target: " + neo4jDir.getAbsolutePath());
		...
		try(Session session = driver.session()) {
         ...
         // count Reactome edges
            StatementResult result5 = session.run("MATCH ()-[r:AnalysisInteractions]->() RETURN count(r) AS count");
			int countR = 0;
            ...
            System.out.println("[INFO]\tDatabase contains " + countR + " Reactome Analysis edges.");
            System.out.println("[INFO]\tDSMN Step 3 Reactome Analysis interactions done.");
		}
	}
}
