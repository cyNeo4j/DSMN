 ...
 * 
 */
public class FilterInteractionsAll_2 {
...
/*		File dir = new File("data");
		...
		File sparqlResult = new File("sparql-resultA-wdAll-20201116-wpallDev.txt");
	
		// run SparqlQuery_1b_Jenkins to create this file (change name accordingly)
		File sparqlResultB = new File("sparql-resultB-wdAll-20201116-wpallDev.txt");
		
		// run SparqlQuery_1c_Jenkins to create this file (change name accordingly)
		File sparqlResultC = new File("sparql-resultC-wdAll-20201116-wpallDev.txt");
		...
		FilterInteractionsAll_2 fi = new FilterInteractionsAll_2(sparqlResult, sparqlResultC, sparqlResultB, sideMetabolites);

		// output files (add date and change name accordingly)
		File network = new File("network-wdAll-20201116-wpallDev.txt");
		File networkFiltered = new File("network-filtered-wdAll-20201116-wpallDev.txt");
	...
	public FilterInteractionsAll_2(File sparqlResult, File sparqlResultC, File sparqlResultB, File sideMetabolites) {
	...
	public void createNetworkFiles(File network, File networkFiltered) throws Exception {
		...
		while((line = reader.readLine()) != null) { //if a  line is not empty
			...
			String key = source + "_" + target;// + "_" create unique key with source and target, to store value from interaction class in.
			...
			//Columns below may be empty, since SPARQL query retrieves these items as "optional".
			...
			map.get(key).getSourceChebi().add(sourceChebi); //Add ChEBI source ID is available
			map.get(key).getTargetChebi().add(targetChebi); //Add ChEBI target ID is available
			map.get(key).getSourceHMDB().add(sourceHMDB); //Add HMDB source ID is available
			map.get(key).getTargetHMDB().add(targetHMDB); //Add HMDB target ID is available
...
*/	
			...
			if((!interactionID.isEmpty()) && (map.get(key).getInteractionID().isEmpty())){
				...
				map.get(key).getInteractionID().add("ReactionID_All:" + count); //Only add a reaction ID number, if none is present.
	...
	private int writeInteractionFile(File out, boolean filtered, Map<String, Interaction> interactions, List<String> sideMetaboliteList) throws Exception {
		...
		for(String s : interactions.keySet()) {
			...
			if(!filtered || (filtered && (!sideSoure && !sideTarget))) {
				...
				writer.write(i.getProteinName().toString().replace("[]","[NA]") + "\t" + i.getProteinID().toString().replace("[","").replace("]", "") +  "\t" +  i.getInteractionID().toString().replace("[]","[NA]").replace("[","").replace("]", "") + "\t");
